...
export default function SplashScreen({ onComplete }: SplashScreenProps) {
  ...
  const [loadingText, setLoadingText] = useState('جاري بدء تشغيل المنصة...');

  useEffect(() => {
    // Elegant progressing texts in Arabic
    const textIntervals = [
      { percentage: 15, text: 'جاري تحميل الأجنحة الرقمية...' },
      { percentage: 40, text: 'تكامل بروتوكول قواعد بيانات Google Sheets...' },
      { percentage: 65, text: 'تحميل العناصر التفاعلية ثلاثية الأبعاد...' },
      { percentage: 85, text: 'تهيئة قاعات العرض وتأمين الخوادم...' },
      { percentage: 100, text: 'أهلاً بك في إكسبو مصر للبناء والعقارات 2026 ✨' }
    ];
  ...
  return (
    <div id="splash-screen" className="fixed inset-0 z-[9999] bg-[#030b1a] flex flex-col items-center justify-center overflow-hidden select-none">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-1/4 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-[#d4af37]/10 rounded-full blur-[120px]" />
        ...
        <div className="absolute inset-0 opacity-10 bg-[linear-gradient(to_right,#d4af37_1px,transparent_1px),linear-gradient(to_bottom,#d4af37_1px,transparent_1px)] bg-[size:40px_40px]" />
      </div>

      {/* Floating Sparkles inside splash */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-[20%] left-[15%] text-[#d4af37]/30 animate-pulse">
          <Sparkles className="w-5 h-5" />
        </div>
        <div className="absolute top-[35%] right-[20%] text-[#d4af37]/20 animate-bounce">
          <Award className="w-6 h-6" />
        </div>
        <div className="absolute bottom-[25%] left-[25%] text-[#d4af37]/20">
          <ShieldCheck className="w-5 h-5 animate-pulse" />
        </div>
        <div className="absolute bottom-[30%] right-[15%] text-[#d4af37]/30">
      ...
      <div className="relative text-center max-w-md px-6 flex flex-col items-center">
        ...
        <div className="relative mb-8">
          ...
          <div className="absolute -inset-4 bg-gradient-to-r from-[#d4af37] to-amber-500 rounded-[35px] blur-xl opacity-30 animate-pulse" />
          
          {/* Rotating borders */}
          <div className="absolute -inset-1.5 bg-gradient-to-tr from-[#d4af37] via-transparent to-[#d4af37] rounded-[32px] animate-spin" style={{ animationDuration: '6s' }} />

          {/* Core EM Logo container */}
          <div className="relative w-28 h-28 bg-[#040e24] rounded-[30px] border-2 border-[#d4af37] flex flex-col items-center justify-center shadow-2xl">
            <span className="text-[#d4af37] font-serif text-5xl font-black italic tracking-wider leading-none select-none">
              EM
            </span>
        ...
        <div className="space-y-2 mb-12">
          <h1 className="text-2xl sm:text-3xl font-black text-transparent bg-clip-text bg-gradient-to-l from-white via-slate-100 to-amber-200 tracking-tight">
            إكسبو مصر للبناء والعقارات 2026
          </h1>
        ...
        <div className="w-72 sm:w-80 bg-brand-blue-dark/50 backdrop-blur rounded-2xl p-4 border border-[#d4af37]/15 shadow-2xl relative">
          ...
          <div className="h-2 w-full bg-[#05142e] rounded-full overflow-hidden border border-white/5 relative">
            <div 
              className="h-full rounded-full bg-gradient-to-r from-[#d4af37] via-amber-500 to-[#d4af37] transition-all duration-150 ease-out shadow-[0_0_12px_rgba(212,175,55,0.4)]"
