...
export default function CountdownTimer() {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl bg-[#d4af37] text-[#030b1a] p-6 md:p-8 shadow-2xl max-w-3xl mx-auto border border-white/20">
      {/* Visual background pattern */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full blur-2xl pointer-events-none"></div>

      <div className="relative z-10 text-center">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-[#030b1a]/10 border border-[#030b1a]/20 rounded-full text-[#030b1a] text-xs font-bold mb-4 animate-pulse">
          <span className="w-1.5 h-1.5 rounded-full bg-[#030b1a]"></span>
          <span>فرصة رقمية محدودة — متبقي نسبة قليلة من الأجنحة المتاحة</span>
        </div>

        <h3 className="text-lg md:text-2xl font-black text-[#030b1a] mb-6 tracking-tight">
        ...
        {timeLeft.isEnded ? (
          ...
          <div className="grid grid-cols-4 gap-2 sm:gap-4 max-w-xl mx-auto">
            {timeUnits.map((unit, index) => (
              ...
              >
                <div className="relative w-full aspect-square xs:w-16 xs:h-16 sm:w-20 sm:h-20 bg-[#030b1a] border border-white/10 rounded-2xl flex items-center justify-center shadow-lg overflow-hidden group">
                  {/* Glowing line indicators */}
                  <div className="absolute top-0 left-0 right-0 h-[3px] bg-[#d4af37]"></div>
                  
                  <span className="text-2xl sm:text-3xl md:text-4xl font-extrabold font-mono text-[#d4af37] tracking-widest leading-none">
                    {String(unit.value).padStart(2, '0')}
                  </span>
                </div>
                <div className="mt-2 text-xs font-bold text-[#030b1a]">{unit.label}</div>
                <div className="text-[9px] text-[#030b1a]/60 font-mono tracking-wider uppercase mt-0.5">{unit.subtext}</div>
              </div>
            ))}
          </div>
        )}

        <div className="mt-6 text-xs text-[#030b1a]/70 font-semibold max-w-lg mx-auto font-sans">
